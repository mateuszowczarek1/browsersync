    ...
    <Layout>
        <Panel :title="`Catalogue: ${catalogue.is_published ? catalogue.name : 'cannot be shown'}`" class="col-span-2">
            <div v-if="catalogue.is_published" class="space-y-2 text-center">
                ...
                <h3 v-if="catalogue.original_author && catalogue.original_author !== catalogue.user.name">This catalogue was cloned from: {{ catalogue.original_author }}</h3>
                <h3 class="text-xl font-semibold">Created at: {{ parsedDate }}</h3>
                <h4 class="text-xl font-semibold">Is published: {{ catalogue.is_published ? 'Made public' : 'Private' }}
                </h4>
                <div class="border-t-2 border-dashed py-2 text-left">
                    <ul v-for="bookmark in catalogue.bookmarks" class="my-5 mx-4 break-words">
                        <li class="rounded-xl bg-purple-950 p-4 transition-colors duration-1000 hover:bg-purple-700 ">
                            ...
                            <p class="mt-2 cursor-copy border-purple-600 border-t text-purple-400"
            ...
            <div v-if="auth.getUser() && auth.getUser().id === catalogue.user_id" class="bg-white/10 rounded-xl my-5">
                ...
                <div class="flex flex-wrap gap-4">
                    <Link :href="`/catalogues/edit/${catalogue.id}`" as="button" method="get"
                        class="py-2 px-4 rounded-xl bg-purple-700 inline-block cursor-pointer hover:bg-purple-600 transition-colors duration-400 mx-4 mb-5">
                    Edit it here</Link>
